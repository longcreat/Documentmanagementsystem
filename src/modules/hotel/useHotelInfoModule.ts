...
const collectCustomSections = (fields: DocumentField[]) => {
  const baseSet = new Set(HOTEL_BASE_SECTIONS);
...
interface UseHotelInfoModuleParams {
  document: Document;
...
export interface HotelInfoModuleResult {
  isActive: boolean;
...
export const useHotelInfoModule = ({
  document,
  fields,
  setFields,
  setOpenSections,
  ...
  isActive,
}: UseHotelInfoModuleParams): HotelInfoModuleResult => {
